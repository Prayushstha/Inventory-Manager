...
function I(e, t) {
	...
	M.prepare("UPDATE products SET name = ?, images = ? WHERE id = ?").run(n, r, e);
	for (let t of i) t.id ? M.prepare("UPDATE variants SET bucket_size = ?, landing = ?, sales = ?, mp = ? WHERE id = ?").run(t.bucket_size, t.landing, t.sales, t.mp, t.id) : M.prepare("INSERT INTO variants (product_id, bucket_size, landing, sales, mp) VALUES (?, ?, ?, ?, ?)").run(e, t.bucket_size, t.landing, t.sales, t.mp);
	...
	for (let t of a) {
		...
		for (let r = 0; r < t.stocks.length; r++) {
			...
			i && M.prepare("INSERT INTO base_stock (base_id, variant_id, stock) VALUES (?, ?, ?)").run(n, i.id, t.stocks[r]);
		}
	}
}
function L(e) {
...
function _e(e) {
	...
	} else n = e === "all" ? "0000-01-01" : `${t.getFullYear()}-${String(t.getMonth() + 1).padStart(2, "0")}-01`;
