...
export function addProduct(product) {
  ...
  for (const base of bases) {
    const baseResult = db.prepare(`INSERT INTO bases (product_id, name) VALUES (?, ?)`).run(id, base.name);
...
export function getProducts() {
  ...
  for (const product of products) {
    ...
    for (const base of bases) {
      const stocks = db.prepare(`SELECT stock FROM base_stock WHERE base_id = ? ORDER BY variant_id`).all(base.id);
